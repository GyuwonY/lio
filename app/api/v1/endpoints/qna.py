...
async def generate_qna(
    ...
    return await service.add_qna_generation_task(
        background_tasks=background_tasks, current_user=current_user
    )
...
@router.put("/list", response_model=List[QnARead], summary="Q&A 벌크 수정")
...
@router.delete("/{qna_ids}", response_model=List[QnARead], summary="Q&A 벌크 수정")
