...
class PortfolioItemPydantic(BaseModel):
    item_type: PortfolioItemType = Field(description="항목 유형 (INTRODUCTION, EXPERIENCE, PROJECT, SKILLS, EDUCATION 중 하나)")
    topic: Optional[str] = Field(None, description="회사명, 프로젝트명 등 항목의 주제")
    start_date: Optional[date] = Field(None, description="업무 또는 프로젝트의 시작일 DD가 없는 경우엔 01로 대체 ex) YYYY-MM-01")
    end_date: Optional[date] = Field(None, description="업무 또는 프로젝트의 종료일 DD가 없는 경우엔 01로 대체 ex) YYYY-MM-01")
    content: str = Field(description="항목의 요약되지 않은 원본 내용, 줄의 공백과 줄바꿈은 한줄로 변경합니다.")
...
class LLMService:
    ...
    async def structure_portfolio_from_text(self, text: str) -> List[PortfolioItemPydantic]:
