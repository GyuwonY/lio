...
class ChatSessionService:
    ...
    async def create_session(self, *, portfolio_id: uuid.UUID, user_id: uuid.UUID) -> ChatSession:
    ...
    async def get_session(self, session_id: str) -> ChatContext | None:
        chat_session = await self.chat_session_crud.get_chat_session_by_session_id(session_id=session_id)
        if not chat_session:
            raise 
