...
- 단, 줄바꿈, 연속된 공백 등은 문맥에 맞는 자연스러운 줄바꿈과 공백이 되도록 변환하며, 텍스트 파싱 과정에서 발생한 것으로 보이는 의미 없는 특수문자나 깨진 글자는 제거하거나 정상적인 표현으로 수정하여 문맥을 깔끔하게 만듭니다.
...
- 각 핵심 주제에 대해 메인 질문 1개와 답변 1개를 생성합니다. 다만, 핵심 주제가 간결 명료하거나 전문 분야와 연관성이 없는 경우 메인 문답은 생성하지 않습니다.
- 질문: 식별된 **전문 분야의 관점**에서, 방문객이 가장 궁금해할 만한 전문적인 관점의 뾰족한 질문을 생성합니다. 
- 답변: 챗봇이 참고할 객관적인 데이터로 작성합니다. STAR 기법을 최대한 활용하되, `상황:` 과 같은 구분자는 사용하지 않아야합니다. 원본 content에 명시적으로 언급된 요소만을 참고하여 풍부한 답변을 구성합니다. 언급되지 않은 요소(예: 성과)는 답변에서 과감히 생략합니다. 1인칭 구어체 대신 각 요소를 키워드 중심으로 명확하게 서술합니다.
    - 프로젝트의 배경 및 주어진 상황
...
- **각 메인 문답의 Action 또는 Result에 초점을 맞춰**, 내용을 더 깊이 파고드는 꼬리 질문 1개를 추가로 생성합니다. 다만, 핵심 주제가 간결 명료하거나 전문 분야와 연관성이 없는 경우 꼬리 문답은 생성하지 않습니다.
