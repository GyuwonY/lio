...
최종 출력 (Final Output)
서론이나 부가 설명 없이, 오직 최종 LLMPortfolio JSON 객체만을 코드 블록에 담아 출력하십시오.
"""
...
이 데이터는 포트폴리오 사이트의 챗봇이 사용할 RAG 데이터이므로, 답변의 품질과 구조가 매우 중요합니다. 당신의 창의적인 추론이 아닌, 오직 주어진 정보에만 근거하여 다음 지침을 엄격히 따르십시오.
---
1. 최종 출력 형식 (Output Schema)
아래 스키마를 반드시 준수하는 JSON 객체 하나만을 출력해야 합니다.
{format_instructions}
---
2. 작업 절차 (Step-by-Step Instructions)
...
3단계: 핵심 경험 추출 (Extract Key Themes)
- 입력된 content의 분량을 고려하여 추출할 주제의 개수를 동적으로 조절합니다. 내용이 짧고 단일 경험에 집중한다면 핵심 주제를 1가지, 가능하다면 그 이상 추출합니다. 내용이 풍부하고 여러 성과를 다루고 있다면 핵심 주제를 3가지, 가능하다면 그 이상 추출합니다.

4단계: 메인 문답 생성 (Generate Main Q&A Sets)
- 각 핵심 주제에 대해 메인 질문 1개와 답변 1개를 생성합니다.
- 질문: 식별된 전문 분야의 관점에서, 방문객이 가장 궁금해할 만한 개방형 질문을 생성합니다.
- 답변: 챗봇이 참고할 객관적인 데이터로 작성합니다. STAR 기법을 최대한 활용하되, 원본 content에 명시적으로 언급된 요소만 포함하여 답변을 구성합니다. 언급되지 않은 요소(예: 성과)는 답변에서 과감히 생략합니다. 1인칭 구어체 대신 각 요소를 키워드 중심으로 명확하게 서술합니다.
    - S (Situation): 프로젝트의 배경 및 주어진 상황
    - T (Task): 담당한 구체적인 역할 또는 해결 과제
    - A (Action): 주도적으로 수행한 핵심 행동 및 과정
    - R (Result): 행동으로 인한 구체적인 정량적/정성적 성과

5단계: 꼬리 문답 생성 (Generate Follow-up Q&A Sets)
- 각 메인 문답의 주제를 더 깊이 탐색하기 위한 꼬리 질문 1~2개와 상세 답변을 추가로 생성합니다. 이 문답은 메인 답변의 특정 부분(예: 구체적인 실행 방법, 성과 측정 기준, 문제 해결 과정)에 대해 더 상세한 정보를 제공하는 역할을 합니다.

6단계: 최종 JSON 취합 (Finalize JSON)
- 생성된 모든 LLMQnASet 객체를 qnas 리스트에 담아 최종 LLMQnAGenerationOutput JSON 객체를 완성합니다.
---
...
최종 출력 (Final Output)
서론이나 부가 설명 없이, 오직 최종 LLMQnAGenerationOutput JSON 객체만을 코드 블록에 담아 출력하십시오.
"""
