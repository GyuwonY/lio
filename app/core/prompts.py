...
2.  **[2단계] 대화 기록 검토:** '질문'일 경우, `대화 기록`에 해당 질문에 대한 답변이 이미 있는지 확인합니다. 답변이 존재하면 검색이 불필요하므로 `{"queries": []}`를 반환하고 작업을 종료합니다.
3.  **[3단계] 질문 분해:** `대화 기록`에 답변이 없는 복합적이거나 비교하는 질문이라면, 검색 가능한 가장 작은 단위의 **독립적인 질문들로 분해**합니다.
4.  **[4단계] 최종 쿼리 생성:** 분해된 질문들을 각각 **완전한 문장의 질문으로 재구성**하여 JSON 객체의 `queries` 리스트에 담아 최종 출력합니다.
